...
- 吴清柳：
在完成本编译原理课程设计的过程中, 我主要负责和凌梓超一起完成代码生成部分，其中我学到了很多知识并取得了一定的成果。

1. 我设计并实现了 AST Adapter，其中将抽象语法树 (AST)与符号表信息结合，并由凌梓超根据前面的AST和符号表转换到我设计的AST Adapter, 为后续的代码生成提供了基础。
2. 使用访问者模式 (Visitor) 遍历抽象语法树 (AST)，实现了模块化、易扩展的遍历方法。
3. 开发了代码生成器 (CodeGenerator)，将 AST 转换为目标语言代码，即 C 语言。
4. 编写了多种测试用例，验证了代码生成器的正确性和鲁棒性。
学到的知识和技能：

1. 学习了抽象语法树 (AST) 的概念及其在编译器和解释器中的应用。
1. 学会了使用访问者模式 (Visitor) 遍历抽象语法树 (AST)，提高了代码的模块化程度和可维护性。
1. 学习了如何设计和实现代码生成器 (CodeGenerator)，将 AST 转换为目标语言代码。
1. 学会了使用 Google Test 编写针对不同功能模块的单元测试，提高了代码质量。
不足之处：

1. 代码生成器 (CodeGenerator) 的性能优化尚未达到最佳水平，仍有提升空间。
1. 目前只支持生成 C 语言代码，未来可以支持更多的目标语言和代码风格选项。
1. 测试用例覆盖范围尚有不足，需要扩展到更多的输入场景和边缘情况。

通过这次项目，我对编译原理和代码生成有了更深入的了解。虽然在项目过程中还存在一些不足，但我已经积累了宝贵的经验，为今后在类似项目中遇到的挑战做好了准备。在未来的工作中，我将努力弥补这些不足，持续提升自己的技能和能力。
...
1. AST中包含的信息不足以生成对应的C代码。在函数翻译中，Pascal通过与函数同名变量赋值的方式修改返回值。为了解决这个问题，需要知道哪个变量与函数同名并进行重命名，以支持同名函数的调用。解决方法是设计AST Adapter。在这个过程中，吴清柳和凌梓超一起将语法分析生成的AST结合符号表信息进行补充，从而获得了足够的信息来转换为C代码。

2. 输入输出函数的转换。Pascal中的输入输出函数使用read、write和writeln。这三个函数都不使用占位符，而是直接使用变量和字符串，中间用','隔开。而在C中，不支持这种形式，需要根据语义生成对应的占位符字符串，并附上这些变量。为了解决这个问题，吴清柳和凌梓超一起, 在AST Adapter中利用符号表信息给函数调用、数组下标访问和二元、一元表达式都附上各自的类型。然后，我设计了一个辅助函数PrintFormatString。当遇到输入输出函数时，根据变量、函数调用等的类型信息生成相应的格式化字符串，从而解决了这个问题。
